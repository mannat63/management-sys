...
export async function fetchAdminReportData({ institute_id, dateFrom, dateTo, batchId, studentId }) {
    ...
    const reportData = students.map(student => {
        ...
        // ACCURATE PERIOD-BASED FEE LOGIC
        // 1. Revenue: Confirmed payments RECEIVED between start and end date
        // 2. Pending Dues: Unpaid fees that became due UP TO end date (cumulative arrears)
        ...
        const testDetails = studentResults.map(r => {
            ...
            return {
                ...
                percentage: testObj?.total_marks > 0 ? ((r.marks / testObj.total_marks) * 100).toFixed(1) : 0
            };
        });
